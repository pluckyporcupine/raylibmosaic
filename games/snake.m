    ...
    BeginDrawing()
        ...
        if not gameOver then
            for i:=0 to screenWidth/SQUARE_SIZE do
                dL1:=(SQUARE_SIZE*i+offset.x/2, offset.y/2)
                dL2:=(SQUARE_SIZE*i+offset.x/2, screenHeight-offset.y/2)
                DrawLineV(dL1, dL2, LIGHTGRAY)
            od
            for i:=0 to screenHeight/SQUARE_SIZE do
                dL1:=(offset.x/2, SQUARE_SIZE*i+offset.y/2)
                dL2:=(screenWidth-offset.x/2, SQUARE_SIZE*i+offset.y/2)
                DrawLineV(dL1, dL2, LIGHTGRAY)
            od
